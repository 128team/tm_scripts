    ...
    (function() {
        ...
        function initIframePlayer() {
            ...
            var PICO = {
                ...
                skip90:'<svg viewBox="0 0 24 24"><path d="M18 13c0 3.31-2.69 6-6 6s-6-2.69-6-6 2.69-6 6-6v4l5-5-5-5v4c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8h-2z"/><text x="12" y="14.5" text-anchor="middle" font-size="7" font-weight="700" fill="currentColor" font-family="sans-serif">90</text></svg>',
            ...
            function getSavedSpeed() {
                try { return parseFloat(localStorage.getItem('ymp-speed')) || 1; } catch(ex) { return 1; }
            }
            function saveSpeed(sp) {
                try { localStorage.setItem('ymp-speed', String(sp)); } catch(ex) {}
            }
            function getSavedQuality() {
                try { return localStorage.getItem('ymp-quality') || ''; } catch(ex) { return ''; }
            }
            function saveQuality(q) {
                try { localStorage.setItem('ymp-quality', q); } catch(ex) {}
            }
            ...
            function getSavedTime() {
                var key = getTimeKey();
                if (!key) return 0;
                try { return parseFloat(localStorage.getItem(key)) || 0; } catch(ex) { return 0; }
            }
            function saveTime(t) {
                var key = getTimeKey();
                if (!key) return;
                ...
                try {
                    ...
                    if (toRemove.length > 100) {
                        toRemove.sort(function(a, b) {
                            return (parseFloat(localStorage.getItem(a)) || 0) - (parseFloat(localStorage.getItem(b)) || 0);
                        });
            ...
            function btn(ico) { var b = document.createElement('button'); b.className='ym-p-btn'; b.innerHTML=ico; return b; }
            ...
            function parseSkips() {
                ...
                try {
                    ...
                    while ((m = re.exec(raw)) !== null) {
                        var label = m[1] === 'opening' ? 'Пропустить интро' : m[1] === 'ending' ? 'Пропустить эндинг' : 'Пропустить';
            ...
            var ORIG_SKIP_SELECTORS = '.skip_button, .vjs-skip-button, [class*="skip" i]:not(script):not(style), [class*="Skip"]:not(script):not(style), button[class*="intro"], button[class*="Intro"]';
            ...
            function collectQualities() {
                ...
                // === kodik ===
                document.querySelectorAll('.quality_button, [data-quality], .bespierde_quality_button').forEach(function(el) {
                    var q = el.dataset.quality || el.textContent.trim();
                    if (q && q.length < 15 && !items.some(function(r){return r.label===q;}))
                ...
                var qualityBtn = null;
                document.querySelectorAll('[data-allplay="settings"]').forEach(function(btn) {
                    var t = btn.textContent.toLowerCase();
                    if (t.indexOf('качество') !== -1 || t.indexOf('quality') !== -1) qualityBtn = btn;
                });
                if (qualityBtn) {
            ...
            window.addEventListener('message', function(e) {
                ...
                if (document.getElementById('ym-player') || document.getElementById('ym-p-loading')) return;
                ...
                loading.innerHTML = '<div class="ym-spinner"></div><div>Загрузка видео...</div>';
                document.body.appendChild(loading);

                // кликаем play в оригинальном плеере
                var playSelectors = '.play_button, .vjs-big-play-button, .fp-play, [data-allplay="playpause"], [data-allplay="play"], .allplay__play, .video_play_button, [class*="play-btn"], [class*="play_btn"], .play-button, button[aria-label="Play"]';
                ...
                if (vid) {
                    try { vid.play().catch(function(){}); } catch(ex) {}
                }
                ...
                console.log('[128 Player] запуск воспроизведения, play кнопок:', playBtns.length, 'video:', !!vid);
                ...
                var pollTimer = setInterval(function() {
                    ...
                    if (attempts === 3 || attempts === 7 || attempts === 12) {
                        document.querySelectorAll(playSelectors).forEach(function(b) { b.click(); });
                    }
                ...
                var obs = new MutationObserver(function() { onVideoReady(); });
                obs.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['src'] });
                setTimeout(function() { if (obs) { obs.disconnect(); obs = null; } }, 16000);
            });
            ...
            function openP(video) {
                ...
                var ov = document.createElement('div'); ov.id = 'ym-player';
                var cl = document.createElement('div'); cl.className = 'ym-p-close'; cl.innerHTML = PICO.close;
                var ct = document.createElement('div'); ct.className = 'ym-p-controls';

                var bPlay = btn(PICO.play);
                var prog = document.createElement('div'); prog.className = 'ym-p-prog';
                var pBuf = document.createElement('div'); pBuf.className = 'ym-p-buf';
                var pBar = document.createElement('div'); pBar.className = 'ym-p-bar';
                prog.append(pBuf, pBar);

                var time = document.createElement('span'); time.className = 'ym-p-time';
                time.textContent = fmt(video.currentTime) + ' / ' + fmt(video.duration);

                var vw = document.createElement('div'); vw.className = 'ym-p-vol';
                var bVol = btn(PICO.vol);
                var vol = document.createElement('input');
                vol.type='range'; vol.min='0'; vol.max='1'; vol.step='0.05'; vol.value=String(video.volume);
                vw.append(bVol, vol);

                var savedSpd = getSavedSpeed();
                var targetSpeed = savedSpd;
                var spd = document.createElement('span'); spd.className = 'ym-p-speed'; spd.textContent = savedSpd + 'x';
                ...
                bSkip90.addEventListener('click', function(e) { e.stopPropagation(); video.currentTime = Math.min(video.duration, video.currentTime + 90); });
                ...
                function fillQualityMenu(items) {
                    ...
                    items.forEach(function(qObj) {
                        ...
                        it.addEventListener('click', function(e) {
                            ...
                            qMenu.querySelectorAll('.ym-p-popup-item').forEach(function(x) { x.classList.toggle('active', x === it); });
                            qMenu.classList.remove('open');
                        });
                        if (savedQ && qObj.label === savedQ) {
                            it.classList.add('active');
                            qBtn.textContent = qObj.label;
                            if (!autoClicked && qObj.el) { autoClicked = true; setTimeout(function() { qObj.el.click(); }, 300); }
                        }
                ...
                if (initQ.length) {
                    ...
                    var qIv = setInterval(function() {
                        ...
                        setTimeout(function() {
                            var found = [];
                            document.querySelectorAll('[role="menuitemradio"], [role="menuitem"]').forEach(function(mi) {
                                ...
                                if (m && !found.some(function(f){return f.label===m[1];})) {
                                    found.push({label: m[1], el: mi});
                                }
                            });
                            var backBtn = document.querySelector('[data-allplay="back"], .allplay__menu__back');
                            if (backBtn) backBtn.click();
                            var closeMenu = document.querySelector('.allplay__menu');
                            if (closeMenu) closeMenu.style.display = 'none';
                            setTimeout(function() { if (closeMenu) closeMenu.style.display = ''; }, 50);
                    ...
                    var qIv2 = setInterval(function() {
                        ...
                        if (q.length) { fillQualityMenu(q); clearInterval(qIv2); }
                ...
                SPEEDS.forEach(function(sp) {
                    ...
                    it.addEventListener('click', function(e) {
                        ...
                        saveSpeed(sp);
                        spdMenu.querySelectorAll('.ym-p-popup-item').forEach(function(x) { x.classList.toggle('active', x === it); });
                ...
                ct.append(bPlay, bNext, bSkip90, prog, time, vw, qWrap, spdWrap, bPip, bFs);

                // --- зоны перемотки ±5с (даблклик слева/справа по видео) ---
                var zones = document.createElement('div'); zones.className = 'ym-p-zones';
                var zoneL = document.createElement('div'); zoneL.className = 'ym-p-zone';
                var zoneR = document.createElement('div'); zoneR.className = 'ym-p-zone';
                var icoL = document.createElement('div'); icoL.className = 'ym-p-zone-icon';
                icoL.innerHTML = PICO.rew5 + '<span>−5 сек</span>';
                var icoR = document.createElement('div'); icoR.className = 'ym-p-zone-icon';
                ...
                function onWaiting() {
                    ...
                    bufTimer = setTimeout(function() { bufSpinner.classList.add('visible'); }, 300);
                }
                function onCanPlay() {
                ...
                function flashIcon(ico) {
                    ...
                    setTimeout(function() { ico.classList.remove('flash'); }, 400);
                }
                function flashCenter(svgHtml) {
                    ...
                    centerTimer = setTimeout(function() { centerIco.classList.remove('flash'); }, 500);
                }
                ...
                function zoneInteract(zone, e) {
                    ...
                    var seekFn, ico;
                    if (zone === zoneL) {
                        seekFn = function() { video.currentTime = Math.max(0, video.currentTime - 5); };
                        ico = icoL;
                    } else {
                        seekFn = function() { video.currentTime = Math.min(video.duration, video.currentTime + 5); };
                        ico = icoR;
                    }
                ...
                zoneL.addEventListener('click', function(e) { zoneInteract(zoneL, e); });
                zoneR.addEventListener('click', function(e) { zoneInteract(zoneR, e); });
                ...
                if (savedTime > 0) {
                    function applyTime() {
                        if (video.duration && savedTime < video.duration - 2) video.currentTime = savedTime;
                    }
                    if (video.duration) {
                        applyTime();
                    } else {
                ...
                ov.addEventListener('touchstart', function(e) { e.stopPropagation(); });

                // auto-hide
                var ht;
                function show() {
                    ct.classList.remove('hidden'); cl.style.opacity='1'; ov.style.cursor='';
                    clearTimeout(ht);
                    ht = setTimeout(function() {
                        if (!video.paused) { ct.classList.add('hidden'); cl.style.opacity='0'; ov.style.cursor='none'; }
                    }, 3000);
                }
                ...
                function syncP() {
                    ...
                    if (skipNextFlash) { skipNextFlash = false; return; }
                    // play → показываем ▶, pause → показываем ⏸
                    flashCenter(isPaused ? PICO.pause : PICO.play);
                }
                function onPause() { syncP(); show(); }
                video.addEventListener('play', syncP);
                video.addEventListener('pause', onPause);
                bPlay.addEventListener('click', function() { video.paused ? video.play() : video.pause(); });
                ...
                function onTimeUpdate() {
                    ...
                    var now = Date.now();
                    if (now - lastTimeSave > 5000) { lastTimeSave = now; saveTime(video.currentTime); }

                    // skip: режим 1 — по таймингам из URL (kodik)
                    if (skipRanges.length) {
                        var activeSkip = null;
                        for (var i = 0; i < skipRanges.length; i++) {
                            if (video.currentTime >= skipRanges[i].start && video.currentTime < skipRanges[i].end) {
                ...
                function onProgress() {
                    if (video.buffered.length && video.duration)
                        pBuf.style.width = (video.buffered.end(video.buffered.length-1) / video.duration * 100) + '%';
                }
                ...
                function stopDrag() {
                    ...
                    if (drag) setTimeout(function() { drag = false; }, 50);
                }
                ...
                vol.addEventListener('input', function() { video.volume=+vol.value; video.muted=false; bVol.innerHTML=video.volume===0?PICO.volM:PICO.vol; });
                bVol.addEventListener('click', function() { video.muted=!video.muted; bVol.innerHTML=video.muted?PICO.volM:PICO.vol; });
                bPip.addEventListener('click', function() { document.pictureInPictureElement ? document.exitPictureInPicture() : video.requestPictureInPicture && video.requestPictureInPicture(); });

                function syncFs() { bFs.innerHTML = document.fullscreenElement ? PICO.fsX : PICO.fs; }
                bFs.addEventListener('click', function() { document.fullscreenElement ? document.exitFullscreen() : ov.requestFullscreen(); });
                ...
                function closeP() {
                    ...
                    qIntervals.forEach(function(iv) { clearInterval(iv); });
                    qIntervals.length = 0;
                }
                ...
                function onKey(e) {
                    ...
                    switch(e.key) {
                        ...
                        case 'ArrowLeft': e.preventDefault(); video.currentTime-=5; flashIcon(icoL); break;
                        case 'ArrowRight': e.preventDefault(); video.currentTime+=5; flashIcon(icoR); break;
                        case 'ArrowUp': e.preventDefault(); video.volume=Math.min(1,video.volume+.05); vol.value=video.volume; break;
                        case 'ArrowDown': e.preventDefault(); video.volume=Math.max(0,video.volume-.05); vol.value=video.volume; break;
                        case 'f': document.fullscreenElement ? document.exitFullscreen() : ov.requestFullscreen(); break;
                        case 'p': bPip.click(); break;
                        case 'm': video.muted=!video.muted; bVol.innerHTML=video.muted?PICO.volM:PICO.vol; break;
                        case 's': if (skipBtn.classList.contains('visible')) skipBtn.click(); break;
                        case 'n': video.currentTime = Math.min(video.duration, video.currentTime + 90); break;
                        case 'N': e.preventDefault(); bNext.click(); break; // Shift+N
                        case 'Escape': closeP(); break;
                    }
                }
        ...
        function applyColsCss(cols) {
            ...
            dynCss.textContent =
                '.ym-grid{grid-template-columns:repeat(auto-fill,minmax(' + s.min + ',1fr))!important;}' +
                '.ym-title{font-size:' + s.fs + '!important;}';
        }
        ...
        const STAR  = '<svg viewBox="0 0 20 20"><path d="M13.6 5.7 11.3.8A1.5 1.5 0 0 0 10 0c-.3 0-.5 0-.8.2l-.5.6-2.3 5-5.2.7-.7.3a1.5 1.5 0 0 0 0 2.3l3.7 3.8-1 5.3A1.5 1.5 0 0 0 4.7 20c.3 0 .5 0 .8-.2l4.6-2.5 4.6 2.5c.5.3 1 .3 1.5 0a1.5 1.5 0 0 0 .6-1.6l-.9-5.3L19.6 9c.4-.4.5-1 .3-1.6a1.5 1.5 0 0 0-1.1-1l-5.2-.8Z"/></svg>';
        const HEART = '<svg viewBox="0 0 20 20"><path d="M18.4 2.8a5.3 5.3 0 0 0-4-1.8 5 5 0 0 0-3.1 1.1c-.5.4-1 .8-1.3 1.3-.4-.5-.8-1-1.3-1.3A5 5 0 0 0 5.6 1a5 5 0 0 0-4 1.8A6.4 6.4 0 0 0 0 7c0 1.7.6 3.3 2 5 1.2 1.4 3 2.9 5 4.6l2.2 2a1.2 1.2 0 0 0 1.6 0l2.3-2c2-1.7 3.7-3.2 5-4.7a7.6 7.6 0 0 0 1.9-5c0-1.6-.6-3.1-1.6-4.2Z"/></svg>';
        const GEAR  = '<svg viewBox="0 0 20 20"><path d="m19.41 12.19-1.84-1.44c.02-.22.04-.48.04-.76 0-.27-.02-.53-.04-.76l1.84-1.44a.9.9 0 00.22-1.15L17.72 3.35a.89.89 0 00-1.11-.4l-2.17.87a7.45 7.45 0 00-1.3-.76l-.33-2.3A.89.89 0 0011.92 0H8.08c-.45 0-.83.33-.88.76l-.33 2.31a7.68 7.68 0 00-1.3.75l-2.18-.87c-.41-.16-.9.02-1.1.39L.37 6.65A.9.9 0 00.59 7.81l1.84 1.44c-.03.28-.04.53-.04.75 0 .23.01.47.04.76L.59 12.19a.9.9 0 00-.22 1.15l1.91 3.31a.89.89 0 001.11.4l2.17-.87c.42.3.85.55 1.3.76l.33 2.3a.89.89 0 00.88.77h3.84c.45 0 .83-.33.88-.76l.33-2.31a7.72 7.72 0 001.3-.76l2.18.87a.88.88 0 001.1-.39l1.92-3.32a.9.9 0 00-.22-1.14ZM10 13.33A3.34 3.34 0 016.66 10 3.34 3.34 0 0110 6.67 3.34 3.34 0 0113.34 10 3.34 3.34 0 0110 13.33Z"/></svg>';
        ...
        function parse(li) {
            const d = {
                href: '#', poster: '', title: '', status: null,
                rating: '', score: '', fav: false, buttons: [],
                slug: '', isOngoing: false
            };
            ...
            if (scoreEl) {
                ...
                if (val > 0) d.score = scoreEl.textContent.replace(/[^\d]/g, '') || String(Math.round(val));
            }
            ...
            for (let i = 0; i < allSpans.length; i++) {
                ...
                const junk = sp.querySelectorAll('.nz, [data-rating], [data-balloon], [data-status], [data-id]');
                for (let j = 0; j < junk.length; j++) {
                    const junkTxt = junk[j].textContent;
                    if (junkTxt) txt = txt.replace(junkTxt, '');
                }
                txt = txt.trim();
                if (txt.length > 3 && txt.length < 300 && !/^\d+\.?\d*$/.test(txt) && txt.length > d.title.length) {
                    d.title = txt;
                }
            }
            if (d.rating && d.title.endsWith(d.rating)) d.title = d.title.slice(0, -d.rating.length).trim();
            if (d.score  && d.title.endsWith(d.score))  d.title = d.title.slice(0, -d.score.length).trim();
        ...
        function makeCard(d) {
            ...
            if (d.rating) h += '<span class="ym-rating">' + STAR + ' ' + esc(d.rating) + '</span>';
            if (d.score)  h += '<span class="ym-score">'  + STAR + ' ' + esc(d.score)  + '</span>';
            ...
            h += '<div class="ym-info">';
            h += '<a class="ym-title" href="' + escAttr(d.href) + '">' + esc(d.title) + '</a>';
            ...
            menuBtns.forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    ...
                    if (savedUl) {
                        const origBtn = savedUl.querySelector('button[title="' + CSS.escape(title) + '"][data-id="' + CSS.escape(dataId) + '"]');
            ...
            function processNext(idx) {
                ...
                fetch(location.origin + '/api/anime/' + item.slug)
                    ...
                    .then(function(json) {
                        ...
                        try { sessionStorage.setItem(cacheKey, JSON.stringify(ep)); } catch(e) {}
                        updateEpBadge(item.card, ep);
                    })
                    .catch(function(err) { console.warn('[YMG] episodes fetch failed:', item.slug, err); })
                    .then(function() {
                        setTimeout(function() { processNext(idx + 1); }, 75);
                    });
            }
        ...
        function startObserver() {
            ...
            const observeTarget = savedUl ? (savedUl.parentElement || document.body) : document.body;
        ...
        function onUrlChange() {
            ...
            urlChangeIv = setInterval(function() {
                ...
                if (attempts > 40) { clearInterval(urlChangeIv); urlChangeIv = null; }
            }, 250);
        }
        ...
        function createMenu() {
            ...
            fabImg.src = 'https://raw.githubusercontent.com/128team/assets/main/logo128b.jpeg';
            fabImg.alt = 'YMG';
            fabImg.onerror = function() { fabImg.style.display = 'none'; menuBtn.textContent = '\u229e'; };
            ...
            const gridSw = makeSwitch('Сетка', isOn, function(val) {
                if (val) { if (!turnOn()) gridSw.inp.checked = false; }
                else turnOff();
            ...
            const SIZE_HINTS = { '3': 'Крупные (3 кол.)', '4': '4 колонки', '5': 'Стандарт (5 кол.)', '6': 'Мелкие (6 кол.)' };
            ...
            ['3', '4', '5', '6'].forEach(function(col) {
                ...
                sb.addEventListener('click', function(e) {
                    ...
                    gridCols = col;
                    applyColsCss(col);
                    try { localStorage.setItem('ymg-cols', col); } catch(ex) {}
                    sizeRow.querySelectorAll('.ym-size-btn').forEach(function(b) {
                        b.className = 'ym-size-btn' + (b.textContent === col + 'x' ? ' active' : '');
                    });
                    if (isOn) rebuildGrid();
                });
            ...
            const sidebarSw = makeSwitch('Сайдбар', !sidebarHidden, function(val) {
                sidebarHidden = !val;
                if (sidebarHidden) document.body.classList.add('ym-no-sidebar');
                else               document.body.classList.remove('ym-no-sidebar');
                try { localStorage.setItem('ymg-sidebar', sidebarHidden ? '1' : '0'); } catch(ex) {}
            ...
            try { playerEnabled = localStorage.getItem('ymg-player') === '1'; } catch(ex) {}

            function sendOpenPlayer() {
                var iframes = document.querySelectorAll('iframe');
                iframes.forEach(function(iframe) {
                    try { iframe.contentWindow.postMessage('ym-open-player', '*'); } catch(ex) {}
                });
            }

            const playerSw = makeSwitch('128 Player', playerEnabled, function(val) {
                playerEnabled = val;
                try { localStorage.setItem('ymg-player', val ? '1' : '0'); } catch(ex) {}
                if (val) {
            ...
            window.addEventListener('message', function(e) {
                ...
                if (active) {
                    ...
                    if (nextEp) {
                        nextEp.click();
                        console.log('[128 Player] переключение на серию', nextEp.textContent.trim());
            ...
            let _drag = false, _moved = false, _sx, _sy, _sl, _st;
            ...
            function applyDragMove(clientX, clientY, threshold) {
                const dx = clientX - _sx, dy = clientY - _sy;
                if (Math.abs(dx) > threshold || Math.abs(dy) > threshold) _moved = true;
                menuBtn.style.left   = Math.max(0, Math.min(_sl + dx, window.innerWidth  - menuBtn.offsetWidth))  + 'px';
                menuBtn.style.top    = Math.max(0, Math.min(_st + dy, window.innerHeight - menuBtn.offsetHeight)) + 'px';
            ...
            // Mouse drag
            function onMouseMove(e) { applyDragMove(e.clientX, e.clientY, 3); }
function onMouseUp() {
                dragEnd(
            ...
            menuBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                if (_touchTriggered) { _touchTriggered = false; return; } // Surface: не дублируй
                if (_moved) { _moved = false; return; }                   // drag: не открывай
        ...
        const fixSelectsObserver = new MutationObserver(function() {
            ...
            fixSelectsTimer = setTimeout(function() { fixSelectsTimer = null; fixSelects(); }, 300);
        });
